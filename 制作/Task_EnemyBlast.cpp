...
namespace  EnemyBlast
{
	...
	void  Object::Render2D_AF()
	{
		...
		if (this->motion == Motion::Start || this->motion == Motion::Infinite) {
			this->res->img1->Draw(di.draw, di.src, di.color);
		}
		if(this->motion == Motion::End){
			this->res->img2->Draw(di.draw, di.src, di.color);
		}
	...
	void  Object::Think()
	{
		...
		case Motion::Infinite:
			if (this->moveCnt > 24) { nm = Motion::End; }
			break;
		case Motion::End:
	...
	BChara::DrawInfo  Object::Anim()
	{
		...
		case Motion::Start:
			if (this->animCnt <= 21)	 work = 0;
			else if (this->animCnt <= 42)work = 1;
			else if (this->animCnt <= 53)work = 2;
			else						 work = 3;
			rtv = imageTable[work];
			break;
		case Motion::Infinite:
